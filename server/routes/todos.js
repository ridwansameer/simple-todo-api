  ...
  try {
    ...
    await knex.transaction(async (trx) => {
      for (const userId of user_ids) {
        await trx('todo_assignment').insert({ user_id: userId, todo_id: todoId });
      }
    });
...
TodoRouter.get('/:id/assignments', async (req, res) => {
  try {
    const { id: todoId } = req.params;
    const assignments = await knex('todo_assignment').where({ todo_id: todoId });
...
TodoRouter.delete('/:id/assignments', async (req, res) => {
  try {
    const { id: todoId } = req.params;
    const { user_ids } = createAssignmentSchema.parse(req.body);
    await knex('todo_assignment').where({ todo_id: todoId }).whereIn('user_id', user_ids).delete();

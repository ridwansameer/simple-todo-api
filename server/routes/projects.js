...
const isUserInOrganisationOfProject = async (projectId, userId) => {
  ...
  const user = await knex('user_organisation').where({ user_id: userId, organisation_id }).first();
  return !!user;
};
...
ProjectRouter.post('/:id/todos', async (req, res) => {
  try {
    ...
    const { user } = req;
    const { title, description, due_date, status } = createTodoSchema.parse(req.body);

    const isUserInOrganisation = await isUserInOrganisationOfProject(projectId, user.id);
    if (!isUserInOrganisation) {
      return res.status(403).json({ error: 'User is not in the organisation of the project' });
    }
